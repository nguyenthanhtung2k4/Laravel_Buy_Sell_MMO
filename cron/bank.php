...
$tokenResult = $TN->get_row("SELECT * FROM `bank` WHERE `short_name` = 'Vietcombank'");
if ($tokenResult) {
    $token = $tokenResult['token'];
} else {
    echo "Không tìm thấy token Vietcombank!";
    exit;
}
...
foreach ($result['data'] as $data) {
    ...
    if ($getUser = $TN->get_row(" SELECT * FROM `users` WHERE `id` = '$user_id' ")) {
        if ($TN->num_rows(" SELECT * FROM `invoices` WHERE `trans_id` = '$tranId' ") == 0) {
            ...
            if ($insertSv2) {
                $isCong = PlusCredits($getUser['id'], $amount, "Nạp tiền tự động qua Vietcombank (#$tranId - $comment - $amount)");
                if ($isCong) {
