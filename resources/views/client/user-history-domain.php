<?php
$title = 'Lịch Sử Mua Miền | ' . $TN->site('title');
$body['header'] = '
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="overflow-x-auto">
                    <div class="w-100">
                        <table id="domainTable" class="w-100 dashboard-table text-nowrap table">
                            <thead>
                                <tr>
                                    <th class="py-2 px-4">Tên miền</th>
                                    <th class="py-2 px-4">Nameserver</th>
                                    <th class="py-2 px-4">Số năm</th>
                                    <th class="py-2 px-4">Giá tiền</th>
                                    <th class="py-2 px-4">Ngày đăng ký</th>
                                    <th class="py-2 px-4">Cập nhật</th>
                                    <th class="py-2 px-4">Hạn sử dụng</th>
                                    <th class="py-2 px-4">Trạng thái</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $data = $TN->get_list("SELECT * FROM `tbl_his_domain` WHERE `user_id` = '" . $getUser['id'] . "' ORDER BY `id` DESC");
                                if (count($data) > 0) {
                                    foreach ($data as $row) {
                                        echo '<tr>
                                            <td>' . htmlspecialchars($row['domain']) . '</td>
                                            <td>' . $row['nameserver1'] . '<br>' . $row['nameserver2'] . '</td>
                                            <td>' . $row['years'] . ' năm</td>
                                            <td>' . format_cash($row['price']) . 'đ</td>
                                            <td>' . $row['create_time'] . '</td>
                                            <td>' . ($row['update_time'] ?? '<i>Chưa cập nhật</i>') . '</td>
                                            <td>' . $row['end_time'] . '</td>
                                            <td>' . status_domain($row['status']) . '
                                        </tr>';
                                    }
                                } else {
                                echo '
