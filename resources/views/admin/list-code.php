...
$title = 'Quản Lý Mã Nguồn | ' . $TN->site('title');
$body = [
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex">
                            <a type="button" href="<?=BASE_URL('admin/add-code')?>"
                                class="btn btn-sm btn-primary btn-wave waves-light waves-effect waves-light"><i
                                    class="ri-add-line fw-semibold align-middle"></i> Thêm mã nguồn</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-nowrap table-striped table-hover table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">Tên Mã Nguồn</th>
                                        <th scope="col">Ảnh</th>
                                        <th scope="col">Giá Bán</th>
                                        <th scope="col">Lượt Xem/Lượt Mua</th>
                                        <th scope="col" class="text-center">Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody>
                                     <?php $i=0; foreach ($TN->get_list(" SELECT * FROM `tbl_list_code` WHERE `id` IS NOT NULL ORDER BY id DESC ") as $row) {?>
                                        ...
                                        <td><a class="text-primary" href="<?=BASE_URL('client/view-code/')?><?=$row['code']?>"><?=$row['name']?> [ID: <?=$row['id']?>]</a>
                                        </td>
                                        <td>
                                         <img src="<?=$row['images'];?>" width="100px"></td>
                                        <td class="text-right">
                                            <b style="color:blue;"><?=format_cash($row['price'])?>đ</b>
                                        </td>
                                        <td>
                                            Lượt xem: <b
                                                style="color:red;"><?=$row['view'];?></b><br>
                                            Lượt mua: <b style="color:blue;"><?=$row['sold'];?></b>
                                        </td>
                                        <td class="text-center fs-base">
                                            <a href="<?=BASE_URL('admin/code-edit/');?><?=$row['id'];?>" class="btn btn-sm btn-primary shadow-primary btn-wave waves-effect waves-light" data-bs-toggle="tooltip" data-bs-original-title="Edit">
...
function RemoveRow(id) {
    cuteAlert({
        ...
        message: "Bạn có chắc chắn muốn xóa mã nguồn ID " + id + " không ?",
        confirmText: "Đồng Ý",
        cancelText: "Hủy"
    }).then((e) => {
        if (e) {
            $.ajax({
                url: "<?=BASE_URL('')?>ajaxs/admin/removeCode.php",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: id,
